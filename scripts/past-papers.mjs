...
export function renderPastPapers({page,esc,url,a,eyebrow,filterSelect}) {
  ...
  for(const q of questions) {
    ...
    if(q.contentFile) {
      if(!/^content\/past-papers\/[a-z0-9-]+\.html$/.test(q.contentFile)) throw Error(q.slug+': use content/past-papers/your-slug.html');
      q.content=fs.readFileSync(q.contentFile,'utf8');
      if(!q.content.trim()||/<\/?(?:html|head|body|h1|script|iframe)\b/i.test(q.content)||/\bon\w+\s*=/i.test(q.content))
        throw Error(q.slug+': use an HTML fragment without scripts, page tags or h1 headings');
      // Root-relative asset/link paths in authored content work on GitHub project sites.
      q.content=q.content.replace(/\b(src|href)=(['"])(\/[^'"\s]*)\2/g,(_,attr,quote,value)=>`${attr}=${quote}${esc(url(value))}${quote}`);
    }
  }
  ...
  for(const q of questions) {
    ...
    page(route(q),q.title,q.description,`<section class="shell detail-intro"><div class="breadcrumbs">${a('/past-papers/','Past paper solutions')} <span>/</span> ${esc(q.topic)}</div>${eyebrow(esc(reference(q)))}<h1>${esc(q.title)}</h1><p>${esc(q.description)}</p><div class="detail-tags"><span>${tierLabel(q.tier)}</span><span>${gradeLabel(q.grades)}</span>${formats(q).map(f=>`<span>${f}</span>`).join('')}</div></section><div class="shell solution-layout"><article class="solution-content">${video}${q.content?`<div class="written-solution" data-math><p class="notice" data-math-notice role="status" hidden></p>${q.content}</div>`:''}</article><aside class="solution-aside"><h2>Question reference</h2><dl><dt>Exam board</dt><dd>${esc(q.board)}</dd><dt>Qualification</dt><dd>${esc(q.qualification)}</dd><dt>Exam</dt><dd>${esc(q.session)} ${q.year}</dd><dt>Paper</dt><dd>${esc(q.paper)}</dd><dt>Question</dt><dd>${esc(q.questionNumber)}</dd><dt>Topic</dt><dd>${esc(q.topic)}</dd></dl><p class="small">Difficulty is an estimate. This is an independent explanation, not an official mark scheme.</p>${q.sourceUrl?a(q.sourceUrl,'Original paper ↗','text-link'):''}<p class="spaced">${a('/past-papers/','Browse all solutions ↗','text-link')}</p></aside></div>`, '/past-papers/');
  }
}
